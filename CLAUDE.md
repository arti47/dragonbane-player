...
### Phase 20 — Regression-Test Harness (Priority: MEDIUM — quality infrastructure) *(new phase, NOT started)*
...
- [ ] **Add a committed test harness under `tests/`.** ✅-target: a single `npm test` (or `node tests/run.js`) that boots the app headless and asserts against it.
