...
| 2026-06-26 | **Firebase Auth & RTDB Resilience Diagnostic COMPLETE.** Upgraded `Sync.ensureAuth`, `createCampaign`, `joinCampaign`, and `linkGoogle` (`app.js`) to catch and report exact Firebase authentication failures (`auth/operation-not-allowed` when Anonymous sign-in is disabled, and `auth/unauthorized-domain` when Google OAuth popups block unlisted domains) alongside RTDB permission rejections. Implemented local device UID fallback (`dragonbane.localUid`) so unauthenticated RTDB databases work seamlessly. |
