...
| `tests/` + `package.json` | Dev-only headless regression harness (`npm test`) — Phase 20. `tests/{run,serve,browser}.js` + `tests/specs/*.js`. Not part of the shipped PWA; `node_modules` gitignored. | ✅ COMPLETE — 5 specs / 86 checks (smoke, spillage, derivation, cast, inventory). |
