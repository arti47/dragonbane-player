...
| `tests/` + `package.json` | Dev-only headless regression harness (`npm test`) — Phase 20. `tests/{run,serve,browser}.js` + `tests/specs/*.js`. Not part of the shipped PWA; `node_modules` gitignored. | ✅ COMPLETE — 6 specs / 101 checks (smoke, spillage, derivation, cast, inventory, a11y). |
...
### Phase 21 — GM Screen (Priority: LOW — new play surface, opt-in) *(new phase, NOT started)*
> The schema and Firebase Security Rules were written with `members/{uid}.role: "player" | "gm"` from day one (see §4.1, §11) specifically so a GM view could be added **with zero database migration**. This phase builds that view as a **distinct mode**, never shown to plain players. Strictly additive — existing player UI is untouched.
- [ ] **GM dashboard for a campaign owner.** ✅-target: a campaign GM can see and lightly manage the whole party from one screen.
