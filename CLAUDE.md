...
- **How to Play (Tutorial) compendium** — first Rules category; combined rules primer + app walkthrough covering setup, hero creation, the D20 mechanic, combat, magic, rest/advancement, and running a game **non-solo (group + GM)** and **solo**.
...
- **PWA Updates:** Includes an in-app *"Update available — tap to reload"* toast (with a ✕ dismiss) when the Service Worker detects a new deploy, guaranteeing all players run identical rules and UI. The app **actively polls** for a new worker — on load, whenever it regains focus (reopening an installed home-screen PWA), and hourly — so a resident installed app is prompted promptly rather than waiting for the browser's own periodic check. A new worker is detected whenever `service-worker.js` changes, i.e. when `CACHE_VERSION` is bumped (which every code push does).
...
| `tests/` + `package.json` | Dev-only headless regression harness (`npm test`) — Phase 20. `tests/{run,serve,browser}.js` + `tests/specs/*.js`. Not part of the shipped PWA; `node_modules` gitignored. | ✅ COMPLETE — 9 specs / 148 checks (smoke, spillage, derivation, cast, inventory, a11y, gm, rules7c, playtest). |
...
| Date | Changes |
|---|---|
| 2026-07-14 | **Solo playtest (agent) → fixed all 3 findings.** Installed the `solo-rpg-playtester` subagent (`.claude/agents/`) and ran it exhaustively over every solo-reachable function (5 AUDIT seeds all reached mission-end + a full PLAY session + a 24-check surface sweep, 0 stalls/console errors). It surfaced 3 minor findings, now fixed: **F1** — the NPC AI attack table printed demands like "roll WIL to resist fear" but the default solo loop had no un-gated attribute roll; `Roller.rollNpcAttackTable` now renders an inline **🎲 Roll WIL/INT/…** control (`Roller.playerAttrCheck`) using the hero combatant's real attribute, applying Scared on a failed fear roll. **F2** — Solo-generator foes had no control to strike back (no `weapons[]`, so `Roller.npcAttack` never fired); `solo.js buildFoe` now attaches a weapon (skill parsed from the template, template damage die) so the foe's combat card exposes a real attack + damage applier (Bestiary parity), and the AI table adds a "🎲 Resolve <weapon>" button on attack rows. **F3** — combat help said "Tap Draw initiative" but the button reads "Re-draw" (auto-draw on first add); help reworded. New regression spec `tests/specs/playtest.js` (12 checks) drives the exact repro paths from real app output; harness **148/148 green**; the agent's 5-seed audit re-run stays 5/5, exit 0. SW cache v75. |
| 2026-07-14 | **"How to start / sustain / end a game" — first-session walkthrough + first-run welcome popup.** Users still couldn't tell how to begin, keep going, or wrap up. Added a **🎬 Your first session — Start → Keep playing → End well** accordion (open by default) as the first entry of Rules → 📘 How to Play (`screens.js renderRuleDetail("howtoplay")`): 12 numbered do-this-now steps across the three phases, covering both group and solo. Added a **one-time welcome modal** on first launch (`main.js`, gated by `localStorage dragonbane.welcomed`): the game in three beats (Start / Keep playing / End well) + a **📖 Show me How to Play** button that deep-links and opens the walkthrough; **Got it** dismisses. The test harness sets the `welcomed` flag in `tests/browser.js` so the popup never intercepts interactions. Verified headless: modal shows once, deep-link opens the walkthrough (all 3 phases present), no modal on reload, 0 errors; harness 136/136 green. SW cache v74. |
| 2026-07-14 | **Reachability audit (pass 2) → editable identity on the sheet.** The pregen picker promised heroes could be "rename[d] or adjust[ed] afterwards", but there was **no UI to edit `identity.name`/`appearance`/`memento`** post-creation (only weakness was editable). Added an editable **Name / Appearance / Memento** group to the sheet's Character panel (owner/GM only): Name commits on change (updates the header live), Appearance/Memento save on input. Implemented as single-/multi-row `<textarea>`s — a plain `<input>` reports an unshrinkable `scrollWidth` on 360/390px that tripped the spillage audit; textareas respect their CSS width. Also hardened `.form-field input/textarea` with `min-width:0; box-sizing:border-box`. Read-only viewers still see the static appearance line. Verified headless: fields present, name edit updates header + persists, appearance/memento persist, 0 errors; harness 136/136 green (incl. the sheet/cast-modal spillage checks). Data note: attributes and skill levels remain non-editable by design (fixed at creation / changed only via advancement). SW cache v73. |
| 2026-07-14 | **Reachability audit → fixed two dead-ends on the sheet.** Enumerated every public method (`Sheet`/`Combat`/`Roller`/`GM`/`Screens`) and confirmed each has a live UI call site (no orphaned features; all nav tabs + toggles reachable). Two features were unreachable for a non-caster: (1) **＋ Learn a spell or school** (`Sheet.learnMagic`) lived inside the Magic panel, which only rendered when the hero already knew spells — so a fighter who takes **Magic Talent** (or wants Dracomancy) had no way in; the Magic panel now **always renders** (collapsed + a hint when empty) so the Learn button is always reachable. (2) **Active Spells & Effects** tracker (`Sheet.addEffect`) was gated the same way — now **always shown**, so any hero can track an ally-cast buff. Verified headless on a spell-less Fighter: Magic panel + Learn button + effects tracker all present, learnMagic modal opens, 0 errors; harness 136/136 green. SW cache v72. |
| 2026-07-14 | **Inline glossary — tap-to-define jargon tokens.** Added `GLOSSARY` (term→plain definition) + `gloss(term,label)` to `core.js`, a `.gloss` dotted-underline style, and a delegated click/Enter/Space handler in `main.js` that toasts the definition (works on touch, unlike `title=`). Seeded the densest always-on jargon on the character sheet: **HP**, **WP** (vitals labels), and **Move / damage bonus / Enc. limit** (derived line); `stepper()` now strips tags for its `aria-label`. Mechanism is app-wide via delegation — more tokens can be tagged incrementally. Verified headless: 6 gloss tokens on the sheet, tapping HP shows "Hit Points — …", aria-label clean, 0 errors; harness 136/136 green. SW cache v71. |
| 2026-07-14 | **Newcomer/idiot-proofing pass (audit → fix).** Audited the app for a total novice (never read the rules, never played solo). Fixes: (1) **Tutorial discoverability** — `Screens.openTutorial()` deep-links to Rules → 📘 How to Play (opens + scrolls the accordion); a **📘 How to Play** button now sits on the Heroes screen (empty state adds "New to Dragonbane or solo play? Tap 📘 How to Play first"), and a **📘 New to solo RPGs?** link sits atop the Solo tab. (2) **Solo loop guide** — a "🧭 The solo loop — what to do each scene" accordion (set scene → ask oracle → act → push/fail-forward → log/threads → Mission +5) on the Solo screen. (3) **🔤 Glossary** accordion added to How to Play defining HP/WP/skill/boon/bane/push/condition/Dragon/Demon/kin/heroic ability/round/stretch/shift/advancement mark/oracle. Verified headless: Heroes tutorial button deep-links + opens howtoplay, glossary present, Solo loop accordion + tutorial link render, 0 errors; harness 136/136 green. SW cache v70. |
| 2026-07-14 | **Solo tab: journal completion — threads, NPC roster, per-result ＋Thread, oracle memory.** Closed the remaining §10 journal gaps in `src/solo.js`: journal schema now carries `threads[]` and `npcs[]` (back-filled in `loadJournal`). Added **🧵 Threads** (add / toggle-resolved / delete) and **👥 NPCs met** (name + note) collapsibles to the journal panel; the NPC generator gained a **👥 Remember NPC** button that saves the built foe to the roster. Every roll result now shows both **＋ Log** and **＋ Thread** (the old `logBtn` became `resultBtns`, applied at all 10 result outputs). The Fortune oracle remembers the last **likelihood + question column** (`dragonbane.soloOraclePref`) and restores them on return. Verified headless: manual+result threads persist (2), Remember→1 NPC, oracle pref saved and restored across nav, results show ＋Log/＋Thread, 0 errors; harness 136/136 green. SW cache v69. |
| 2026-07-14 | **Solo tab: seamless-loop pass — journal, linked-hero strip, one-tap combat handoff.** Audit found the Solo tab was a set of isolated rollers, not the guided loop §10 specifies. Added to `src/solo.js`: (1) **📓 Solo Journal & Scene** — a persisted scene note + running log keyed per linked hero (`dragonbane.soloJournal:<heroId|global>`; `journalKey/loadJournal/saveJournal`), with a **＋ Log** button on every roll result (Oracle, Inspiration, Twist, NPC action, Shift, Camp, Forage, Journey Mishap) plus manual add/delete/clear. (2) **Linked-hero strip** — glanceable ❤️HP/⚡WP with **Open sheet**, quick **Round/Stretch/Shift** rests (inline `Store.update`, updates in place), and **🏅 Mission +5** (opens sheet → `soloMissionMarks`). (3) **⚔ Fight it** in the NPC generator — drops the foe *and* the linked hero (vitals/armor via `effHpMax/effWpMax/equippedArmor`) into the shared Combat tracker and jumps to the Combat tab. Solo help accordion updated. New imports: `Sheet`, `effHpMax/effWpMax/equippedArmor`, `confirmModal`. Verified headless: strip vitals + Shift rest→full, Oracle ＋Log persists an entry, Fight handoff yields [hero,npc] on the party tab, 0 errors; harness 136/136 green. SW cache v68. |
| 2026-07-14 | **Per-screen "❓ How to use" help accordions on every screen.** Added `helpBox(title, steps)` to `core.js` (collapsed `<details>`, bullet steps as trusted HTML). Injected a context-specific, collapsed-by-default help accordion at the top of each screen: Heroes (4), Combat tracker (5), Solo Assistant (5), GM Screen (5), Rules library (4), Settings & About (4), and the Character sheet (5) — step-by-step bullets on what each control does. Always shown, expand/close, no persisted state. Verified headless: all 7 screens render their accordion, collapsed by default, 0 errors; harness 136/136 green. SW cache v67. |
| 2026-07-14 | **Rules library: added a "📘 How to Play (Tutorial)" accordion (first category).** Combined rules primer + app walkthrough rendered in `screens.js renderRuleDetail("howtoplay")` (prose like the `stages`/`journeys` entries — no data.js table). Nested `<details>`: ① Setup/storage, ② Make a hero, ③ Core D20 roll mechanic, ④ Combat, ⑤ Magic, ⑥ Rest/death/advancement, ▶ Running a NON-SOLO game (campaign create/join, party sync, GM Screen, loop), 🧭 Running a SOLO game (Solo Mode toggle, oracle/inspiration/twists/NPC gen/journey tools, link-a-hero, solo loop + mission marks). Added as first entry in the `Screens.rules` cats array; searchable by the existing live filter. Verified headless: first accordion is `howtoplay`, contains both SOLO + NON-SOLO sections, search "oracle" surfaces it, 0 errors; harness 136/136 green. SW cache v66. |
| 2026-07-14 | **Solo tab: link a hero so journey/skill rolls use the real sheet + full dice engine.** Previously every Solo journey check (Bushcraft, Forage/Hunt, and the WIL/CON mishap follow-ups) had a manual number box — because the Solo tab isn't bound to a character, you had to copy the value off your sheet, and the roll ignored conditions/banes/pushing/advancement. Added a **"🎲 Rolling as" hero picker** at the top of the Solo tab (`src/solo.js`; selection persisted in `localStorage` `dragonbane.soloHeroId`, exposed via `SoloMode.heroId/linkedHero/setHero`). When a hero is linked: **Camp & Rest** and **Foraging & Hunting** route through the real `Roller.skill(hero.id, …)` modal (conditions, boons/banes, pushing, auto-advancement); Forage gets a skill dropdown (Bushcraft / Hunting & Fishing) pulled from the sheet; on success the panel echoes the rations, on a failed camp it rolls a Journey Mishap. The **WIL/CON mishap follow-ups** read the hero's real attribute and, on a failure, apply the condition governed by that attribute (WIL→Scared, CON→Sickly) straight to the sheet via `applyInvoluntaryConditionTo`. With **no hero linked** the old manual-entry inputs remain (GM/unbound use). Verified headless: picker selects the hero, Camp opens the real Bushcraft roller, Forage lists both skills w/ levels, a failed WIL roll writes Scared to the hero; harness 136/136 green. SW cache v65. |
| 2026-07-14 | **Solo tab: Journey Mishaps now offer their follow-up attribute roll inline.** Several journey mishaps call for a saving roll — "Restless Spirits … (Fear attack; roll **WIL** or gain Scared)", "Sudden Downpour … (Cold condition; roll **CON** or suffer)". A rolled mishap now parses its effect text for `roll <ATTR>` and, when present, renders an inline **🎲 Roll WIL/CON/…** control (editable attribute value, since the Solo tab isn't bound to a character) that resolves d20 ≤ attr and reports resisted vs. "failed — you <consequence>" (the consequence parsed from the "…or <x>" clause; Dragon/Demon flagged). Refactored the mishap display into a shared `mishapNode(mp)` DOM builder used by **both** the "🎲 Roll Journey Mishap (D6)" button and the Camp & Rest failure path (`src/solo.js` `SoloMode.view`), so a failed camp roll surfaces the same interactive follow-up. Verified headless: d6=6 → Roll WIL (forced 20 → "failed — you gain Scared"), d6=1 → Roll CON, d6=3 (Wild Beast) → no attr button; 0 errors; harness 136/136 green. SW cache v64. |
| 2026-07-14 | **Solo tab: renamed the Journeys card + added a Journey Mishap header.** The card is now interactive rollers, not a static reference, so its title changed from "🌲 Wilderness Journeys & Travel **Reference**" to "🌲 Wilderness Journeys & Travel **Tools**" (`src/solo.js`). Added an intro header line above the "🎲 Roll Journey Mishap (D6)" button so it matches the ⛺/🍄 sections: **🌩️ Journey Mishap:** Bad luck befalls the party while travelling or resting. Copy/labels chosen with the user. Verified headless (title + header render, 0 errors); harness 136/136 green. SW cache v63. |
| 2026-07-14 | **Solo tab: removed the "View full Journey Mishap Table (D6)" accordion.** Dropped at the user's request — the "🎲 Roll Journey Mishap (D6)" button still resolves the die and shows the rolled result in its output box, so the full static list under the collapsible was redundant. Removed the `<details>` wrapper plus the `mishapList`/`renderMishapList` highlight helper from `src/solo.js` (`SoloMode.view`); the roll no longer highlights a row in a list (there is no list), it just displays the rolled entry. `DB.journeyMishaps` remains the single source of truth (still rendered in full by the Rules-browser "Wilderness Journeys" compendium). Verified headless: accordion gone, roll still outputs an entry, 0 errors; harness 136/136 green. SW cache v62. |
| 2026-07-14 | **PWA update toast now fires promptly for installed home-screen apps & browser tabs.** The "update available" toast already existed but only appeared when the browser happened to re-check the service worker — which for a resident installed PWA can be many hours (or not at all while it stays open). Reworked the registration in `src/main.js` to **actively poll** for a new worker: on load, on `visibilitychange`→visible and window `focus` (so reopening the home-screen app checks immediately), and hourly via `setInterval`. Also handles an already-`waiting` worker at registration, registers with `updateViaCache:"none"`, keeps the very first install silent (only prompts when a controller already exists), and the toast gained a ✕ **dismiss** button (styled in `styles.css` + `env(safe-area-inset-top)` so it clears the notch). A new worker is detected whenever `service-worker.js` changes — i.e. when `CACHE_VERSION` is bumped, which every push already does (§9). Verified end-to-end headless: SW takes control → no toast on first install → simulating a deploy (bumped version) + `reg.update()` makes the toast appear, 0 page errors; harness 136/136 green. Updated §4.2. SW cache v61. |
| 2026-07-14 | **Solo tab: made the whole Wilderness Journeys & Travel panel rollable.** The Journey reference was the only static panel on the Solo screen; rebuilt it (`src/solo.js` `SoloMode.view`) with interactive rollers matching the other solo widgets: **⏱️ Shifts** → a "🎲 Random shift (D4)" roller (Morning/Day/Evening/Night); **⛺ Camp & Rest** → a Bushcraft roll-under check (editable skill level; Dragon/Demon flagged) that on failure **auto-rolls the Journey Mishap Table** into the same output; **🍄 Foraging & Hunting** → a Bushcraft/Hunting roll-under that yields **D6 rations** on success (+D6 on a Dragon), nothing on failure; and **🎲 Journey Mishap Table (D6)** → a roll button that highlights the rolled row in the full list. Moved the mishap table into the data layer as `DB.journeyMishaps` (D6, `{d6, effect}`) per §9 single-source-of-truth — it was hardcoded in both `solo.js` and `screens.js`; both now read from it (the Rules-browser "Wilderness Journeys" compendium renders the list from the data). SW cache v60. |
| 2026-07-02 | **Fix: a campaign GM couldn't turn the GM Screen tab off.** `GM.enabled()` was `Settings.gmScreen() || (campaign role === "gm")`, so the OR force-showed the 🎲 GM tab for a synced campaign GM no matter how the Settings toggle was set (the switch even read "off" while the tab stayed). Made it **tri-state**: an explicit user choice (`gmScreen` stored `true`/`false`) always wins; only when unset does it default to on for a campaign GM. The Settings toggle now reflects/controls the *effective* state (`GM.enabled()`), so flipping it off writes an explicit `false` and hides the tab for everyone, GM included. Verified in a headless browser: default hidden → toggle on shows the tab (`gmScreen:true`) → toggle off hides it (`gmScreen:false`), 0 console errors; `gm`+`smoke` specs green. SW cache v59. |
