...
export const Roller = {
    // Roll a d20 honoring a net boon/bane (+ = boon → take lowest; − = bane → take
    // highest). Boons/banes stack: each net step adds a die, so |net| = 2 rolls
    // 3d20 and keeps the best/worst (boons/banes already cancel one-for-one in net).
    d20net(net) {
      const dice = Array.from({ length: 1 + Math.abs(net) }, () => Dice.d(20));
    ...
    skill(charId, name, opts = {}) {
      ...
      const doRoll = (pushedCondition) => {
        ...
        if (!success && !demon && !pushedCondition) {
          ...
          if (!remaining.length) {
            pushWrap.querySelector("p").textContent = hasSS
              ? "All six conditions held — only Sole Survivor lets you push:"
              : "All six conditions held — you cannot push this roll.";
          }
    ...
    renderDamageApplier(attackerCombatantId, rawDamage, defaultIgnoreArmor = false, noAdvance = false) {
      ...
      applyBtn.onclick = () => {
        ...
        if (!noAdvance && attackerCombatantId) {
          const att = comb.combatants.find(c => c.id === attackerCombatantId);
          if (att) att.acted = true;
        }
        Combat.save(comb);
        if (!noAdvance && attackerCombatantId) Combat.advanceTurn(attackerCombatantId);
    ...
    heroWeaponAttack(charId, weapon, combatantId) {
      ...
      const doRoll = (isPush) => {
        ...
        if (crit) {
          // Dragon on an attack: the attacker chooses ONE critical effect.
          // Melee: double damage / free second attack on another enemy / ignore
          // armor (piercing). Ranged: double damage / ignore armor.
          ...
          dblChip.onclick = () => setChoice(dblChip, "double");
          armChip.onclick = () => setChoice(armChip, "ignore");
          chips.append(dblChip, armChip);
          if (weapon.type !== "ranged") {
            const secChip = el(`<button class="skill-chip">⚔ Free second attack</button>`);
            secChip.onclick = () => setChoice(secChip, "second");
            chips.appendChild(secChip);
          }
          choiceWrap.appendChild(chips);
          out.appendChild(choiceWrap);
        } else if (success) {
    ...
    // ---- Monster activation: roll the D6 attack table once per Ferocity ----
    // A monster makes Ferocity attacks per turn (Demon 2, Dragon 3, …). Rolls the
    // table that many times in one modal. Damage appliers don't auto-advance the
    // turn (so all attacks can be applied); the activation is marked acted once.
    monsterTableRoll(cb) {
      const all = typeof DRAGONBANE_MONSTERS !== "undefined" ? DRAGONBANE_MONSTERS : [];
      const fero = Math.max(1, cb.ferocity || (all.find((x) => x.id === cb.monId) || {}).ferocity || 1);
      if (cb.id) {
        const comb = Combat.load();
        const att = comb?.combatants?.find((x) => x.id === cb.id);
        if (att && !att.acted) { att.acted = true; Combat.save(comb); const nav = document.querySelector("#app-nav button.active"); if (nav && nav.dataset.route === "party") Combat.rerender(); }
      }
      if (fero <= 1) { const d6 = Dice.d(6); const idx = Math.min(cb.attacks.length - 1, Math.floor((d6 - 1) / (6 / cb.attacks.length))); return this.monsterAttack(cb.name, cb.attacks[idx] || cb.attacks[0], d6, cb.id); }
      const m = modal(`${cb.name}: Ferocity ${fero} — ${fero} attacks this turn`);
      for (let k = 0; k < fero; k++) {
        const d6 = Dice.d(6);
    ...
    cast(charId, spell, isTrick) {
        ...
        if (isHeal) {
          ...
          [4, 6, 8, 10, 12, 20].forEach(d => {
            ...
            db.onclick = () => {
              ...
              // this very cast — any excess is lost afterward (clamped in doCast).
              Store.update(charId, ch => {
                ch.state.hp = Math.max(0, ch.state.hp - roll);
                ch.state.wp = (ch.state.wp || 0) + roll;
              });
              showToast(`🩸 Power from the Body (Rolled D${d})!\nTook ${roll} damage, gained ${roll} WP — spend it now, any excess is lost.`, "error");
              m.close();
      ...
      if (isTrick) {
        ...
        if (hasMetal) { btn.disabled = true; btn.style.opacity = "0.4"; btn.style.cursor = "not-allowed"; btn.title = "Remove metal armor/weapon to cast"; }
        btn.onclick = () => {
          if (hasMetal) return;
      ...
      const doCast = (pushedCondition) => {
        ...
        // Deduct the cost; clamp back to max so any temporary Power-from-the-Body
        // excess WP is lost after this cast (per the rule).
        if (!pushedCondition) Store.update(charId, (ch) => { ch.state.wp = Math.min(effWpMax(ch), Math.max(0, ch.state.wp - cost)); });
        ...
        if (!success && !demon && !pushedCondition) {
          const pushIntro = remaining.length
            ? "Push? Choose a condition, then re-roll (WP already spent):"
