...
import { DB, Dice } from './core.js';
import { classifyItem, normName, resolveArmorItem, resolveCanonicalSpell, resolveHelmetItem } from './rules.js';

// Apply an INVOLUNTARY condition (monster ability, failed-spell mishap, fear,
// disease) to a character object (mutates ch.state). Per the rules (CLAUDE.md
// §7C F2): if the preferred condition is free, gain it; if it's already held,
// you gain another condition instead ("choose another"); if all six are already
// held, you instead lose D6 WP (or D6 HP if WP is 0). Returns a label describing
// what happened. (Voluntary pushing uses a different rule — it is simply blocked
// once all six are held.) The caller wraps this in a Store.update.
export function applyInvoluntaryConditionTo(ch, preferredKey) {
  if (!ch.state.conditions) ch.state.conditions = {};
  const conds = ch.state.conditions;
  const all = DB.conditions || [];
  const unheld = all.filter((cn) => !conds[cn.key]);
  if (!unheld.length) {
    const d = Dice.roll("D6");
    if ((ch.state.wp || 0) > 0) { ch.state.wp = Math.max(0, ch.state.wp - d); return `all six conditions held → lost ${d} WP`; }
    ch.state.hp = Math.max(0, (ch.state.hp || 0) - d); return `all six conditions held (WP 0) → lost ${d} HP`;
  }
  const targetKey = !conds[preferredKey] ? preferredKey : unheld[0].key;
  conds[targetKey] = true;
  const name = (all.find((c) => c.key === targetKey) || {}).name || targetKey;
  return `gained ${name}`;
}
...
export function normalizeInventory(c) {
    ...
    if (typeof c.state.stretchRestUsed !== "boolean") c.state.stretchRestUsed = false;
