...
export const Combat = {
    ...
    draw(s) {
      ...
      const extras = [];
      // Everyone draws one card. Veteran (retain last card) and Lightning Fast
      // (draw two, choose one) are ACTIVATED abilities the player triggers via
      // row controls — they aren't auto-applied (CLAUDE.md §7C F7/F8/F9), so a bad
      // auto-kept card can't hurt and their WP cost is only paid when used.
      s.combatants.forEach((cb) => {
        const priorCard = cb.init; // last round's card, for an opt-in Veteran retain
        cb.init = nextCard();
        cb.prevInit = (priorCard == null ? null : priorCard);
        cb.done = false; cb.acted = false;
        cb.initUsed = false; // per-round flag: a Veteran/Lightning Fast activation used this round
        if (cb.kind === "hero" && cb.charId && has(cb, "Army of One")) {
          extras.push({ ...cb, id: uid(), init: nextCard(), done: false, acted: false, prevInit: null, initUsed: false, name: `${cb.name} (Turn 2)`, isArmyOfOneSecondary: true });
        }
      });
    ...
    // Veteran (1 WP, once/round): retain last round's initiative card instead of
    // this round's. Opt-in so a worse new card can't be forced on the player.
    useVeteran(combatantId) {
      const s = this.load();
      const cb = s.combatants.find((c) => c.id === combatantId);
      if (!cb || !cb.charId) return;
      if (cb.initUsed) { showToast("Already used an initiative ability this round.", "error"); return; }
      if (cb.prevInit == null) { showToast("No previous card to retain (Veteran works from round 2).", "error"); return; }
      const ch = Store.get(cb.charId); if (!ch) return;
      if ((ch.state.wp || 0) < 1) { showToast("Not enough WP — Veteran costs 1.", "error"); return; }
      Store.update(cb.charId, (c2) => { c2.state.wp = Math.max(0, c2.state.wp - 1); });
      const kept = cb.prevInit;
      this.mutate((st) => { const r = st.combatants.find((c) => c.id === combatantId); if (r) { r.init = kept; r.initUsed = true; r.done = false; } });
      showToast(`Veteran: kept card ${kept} (−1 WP).`);
    },
    // Lightning Fast (2 WP, once/round): draw a second card and choose which to keep.
    useLightningFast(combatantId) {
      const s = this.load();
      const cb = s.combatants.find((c) => c.id === combatantId);
      if (!cb || !cb.charId) return;
      if (cb.initUsed) { showToast("Already used an initiative ability this round.", "error"); return; }
      const ch = Store.get(cb.charId); if (!ch) return;
      if ((ch.state.wp || 0) < 2) { showToast("Not enough WP — Lightning Fast costs 2.", "error"); return; }
      const cardA = cb.init, cardB = Dice.d(10);
      const m = modal(`Lightning Fast — ${cb.name}`);
      m.body.appendChild(el(`<p class="stat-line">Draw a second card and keep the one you prefer (lower acts earlier). Costs 2 WP.</p>`));
      const row = el(`<div class="rest-row"></div>`);
      const pick = (card) => {
        Store.update(cb.charId, (c2) => { c2.state.wp = Math.max(0, c2.state.wp - 2); });
        this.mutate((st) => { const r = st.combatants.find((c) => c.id === combatantId); if (r) { r.init = card; r.initUsed = true; r.done = false; } });
        m.close(); showToast(`Lightning Fast: kept card ${card} (−2 WP).`);
      };
      const a = el(`<button class="btn">Keep ${cardA} (drawn)</button>`); a.onclick = () => pick(cardA);
      const b = el(`<button class="btn">Keep ${cardB} (new)</button>`); b.onclick = () => pick(cardB);
      row.append(a, b); m.body.appendChild(row);
    },
    view() {
      const s = this.load();
      ...
      if (monsters.length) {
        ...
        monAdd.onclick = () => {
          ...
          this.mutate((st) => st.combatants.push({
            ...
            hp: m.hp, maxHp: m.hp, armor: m.armor, attacks: m.attacks, ferocity: m.ferocity || 1
          }));
        };
      ...
      ord.forEach((cb) => {
        ...
        if (cb.kind === "monster" && cb.attacks && cb.attacks.length) {
          ...
          if (h) {
            ...
            // Initiative abilities (Phase 11 / §7C F7–F9) — activated, opt-in, WP-charged.
            const hasVet = (h.abilities || []).some((a) => a.name === "Veteran");
            const hasLF = (h.abilities || []).some((a) => a.name === "Lightning Fast");
            if ((hasVet || hasLF) && cb.init != null && !cb.isArmyOfOneSecondary) {
